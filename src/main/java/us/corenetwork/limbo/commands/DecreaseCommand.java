...
public class DecreaseCommand extends BaseLimboCommand {

	public DecreaseCommand()
	{
		permission = "decrease";
		desc = "Decrease time in Limbo";
		needPlayer = false;
	}
	...
	public void run(CommandSender sender, String[] args)
	{
		...
		{
			Util.Message("Usage : /lim decrease [<player>] <time> [silent]", sender);
			return;
		}
		...
		if(args.length == 1)
		{
			...
			{
				Util.Message("You have to execute /lim decrease <time>  as player", sender);
				return;
			}
		...
		if(args.length == 2)
		{
			if(args[1].toLowerCase().equals("silent"))
			{
				...
				{
					Util.Message("You have to execute /lim decrease <time> silent  as player", sender);
					return;
				}
		...
		if(silent == false)
		{
			if(self)
			{
				Util.Message(Settings.MESSAGE_DECREASED_SELF.string().replace("<Time>", time), sender);
			}
			else
			{
				Util.Message(Settings.MESSAGE_DECREASED_SELF.string().replace("<Time>", time), player);
				Util.Message(Settings.MESSAGE_DECREASED.string().replace("<Player>", player.getName()).replace("<Time>", time), sender);
			}
		}
